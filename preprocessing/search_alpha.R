...
search_alpha = function(data, Kt = 10,  forcing_functions = c("int", "x"),
	...
	get_alpha_loss = function(data, alpha0){
		...
		##################################################
		# find beta
		xstar_ls = lapply(x_data, make_Xstar, alpha = alpha0, spline_basis = spline_basis, time = basis_time)
		...
		alpha_loss(data, x_data, beta_cur, alpha0)
	}
	...
	best_alpha = alpha_grid2[which.min(loss)]
	...
	list(best_alpha = alpha_grid3[which.min(loss3)], loss = loss_df)
}
